...
function updateHeartbeat() {
  fetch(API + '/api/heartbeat?_t=' + Date.now()).then(r => r.json()).then(d => {
    ...
    // 自动感知后台运行的推荐任务（页面加载后才启动/server重启后的孤儿任务）：
    // 心跳存在、进程存活且未完成，且推荐按钮未被禁用（未在显示进度）→ 恢复进度条
    var recHb = hb['fund_recommend'];
    if (d.alive && d.alive.fund_recommend && recHb &&
        recHb.phase !== '完成' && recHb.phase !== '失败' && (recHb.overall_pct || 0) < 100) {
...
async function loadSavedTables() {
  // 并行请求推荐表+自选表+心跳，避免串行等待(省约1.2s)
  var rtHtml = null, ftHtml = null, hbR = null;
  try {
    var results = await Promise.all([
      fetch(API + '/api/recommend-table' + '?_t=' + Date.now()).then(function(r){ return r.ok ? r.text() : null; }),
      fetch(API + '/api/fund-table' + '?_t=' + Date.now()).then(function(r){ return r.ok ? r.text() : null; }),
      fetch(API + '/api/heartbeat?_t=' + Date.now()).then(function(r){ return r.json(); }).catch(function(){ return null; })
    ]);
    rtHtml = results[0]; ftHtml = results[1]; hbR = results[2];
  } catch(e) {}
  try {
  ...
  var poll = setInterval(async function() {
    ...
    try {
      ...
      if (recAlive && recData && !recDone) {
        ...
        if (statusEl) statusEl.textContent = '⏳ 获取当日涨跌...';
        ...
        poll = setInterval(async function() {
          ...
          try {
            ...
            if (tdAlive && rec2) {
              ...
              if (statusEl) statusEl.textContent = '📋 刷新涨跌 ' + (rec2.detail || '') + ' | ' + pct + '%';
  ...
  fetch(API + '/api/market-indices?_t=' + Date.now()).then(function(r){ return r.json(); }).then(function(d){
    ...
    for (var i = 0; i < d.indices.length; i++) {
      ...
      if (card) {
        ...
        if (priceEl) priceEl.textContent = idx.price.toFixed(2);
        // 更新点数
        var ptsEls = card.querySelectorAll('[data-role="change"]');
        if (ptsEls.length >= 2) {
          ptsEls[0].textContent = ptsSign + idx.change_points.toFixed(2);
          ptsEls[0].style.color = color;
          ptsEls[1].textContent = sign + idx.change_pct.toFixed(2) + '%';
          ptsEls[1].style.color = color;
        }
      } else {
        // 首次加载或数量变化时回退到重建
        var html = '';
        for (var j = i; j < d.indices.length; j++) {
          var jdx = d.indices[j];
          var jcolor = jdx.change_pct > 0 ? '#ef5350' : (jdx.change_pct < 0 ? '#66bb6a' : '#888');
          var jsign = jdx.change_pct > 0 ? '+' : '';
          var jptsSign = jdx.change_points > 0 ? '+' : '';
          html += '<div style="background:rgba(255,255,255,0.03);border-radius:8px;padding:8px 10px;" data-idx-name="' + jdx.name + '">'
            + '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:4px;">'
            + '<span style="color:#aaa;font-size:12px;">' + jdx.name + '</span>'
            + '<span style="font-family:Consolas;font-weight:600;font-size:14px;color:#e0e0e0;" data-role="price">' + jdx.price.toFixed(2) + '</span>'
            + '</div>'
            + '<div style="display:flex;align-items:center;gap:8px;">'
            + '<span class="marketSparkline" style="display:inline-block;width:160px;height:48px;flex-shrink:0;"></span>'
            + '<span style="font-family:Consolas;font-size:12px;color:' + jcolor + ';" data-role="change">' + jptsSign + jdx.change_points.toFixed(2) + '</span>'
            + '<span style="font-family:Consolas;font-size:12px;font-weight:600;color:' + jcolor + ';" data-role="change">' + jsign + jdx.change_pct.toFixed(2) + '%</span>'
            + '</div>'
            + '<div style="display:flex;align-items:center;gap:8px;margin-top:4px;">'
            + '<span class="marketKline" style="display:inline-block;width:160px;height:36px;flex-shrink:0;"></span>'
            + '<span style="color:#555;font-size:10px;">30日K线</span>'
            + '</div>'
            + '</div>';
        }
...
function _renderMarketSparklines() {
  fetch(API + '/api/market-trends?_t=' + Date.now()).then(function(r){ return r.json(); }).then(function(d){
    ...
    for (var i = 0; i < d.trends.length; i++) {
      ...
      if (t.current_price !== undefined) allPrices.push(t.current_price);
      var mn = allPrices[0], mx = allPrices[0];
      for (var k = 1; k < allPrices.length; k++) {
        if (allPrices[k] < mn) mn = allPrices[k];
        if (allPrices[k] > mx) mx = allPrices[k];
      }
      var range = mx - mn || 1;
      ...
      // 用实时现价（如有）判定颜色，确保和显示数值一致
      var latestPrice = t.current_price !== undefined ? t.current_price : closes[closes.length - 1];
      var lineColor = latestPrice >= basePrice ? '#ef5350' : '#66bb6a';
      var polyPts = svgPts.join(' ');
      var bgPts = polyPts + ' ' + (sw - pad).toFixed(1) + ',' + (sh - pad).toFixed(1) + ' ' + pad.toFixed(1) + ',' + (sh - pad).toFixed(1);
      // 如果后端传了实时现价，在折线末端画一个圆点标记当前价位
      var dot = '';
      if (t.current_price !== undefined) {
        var lastX = parseFloat(svgPts[svgPts.length - 1].split(',')[0]);
        var dotY = sh - pad - (t.current_price - mn) / range * (sh - pad * 2);
        dot = '<circle cx="' + lastX.toFixed(1) + '" cy="' + dotY.toFixed(1) + '" r="2.5" fill="' + lineColor + '" stroke="#fff" stroke-width="0.5"/>';
      }
...
/** 构建K线SVG（迷你版，仅蜡烛） */
function _buildKlineSvg(kl, w, h, onClickIdx) {
  if (!kl || kl.length < 2) return '';
  var mn = Infinity, mx = -Infinity;
  for (var i = 0; i < kl.length; i++) {
    if (kl[i].l < mn) mn = kl[i].l;
    if (kl[i].h > mx) mx = kl[i].h;
  }
  var range = mx - mn || 1;
  var pad = 2;
  ...
  var n = kl.length;
  var candleW = cw / n;
  var bodyW = Math.max(candleW * 0.6, 1.2);
  var parts = [];
  for (var i = 0; i < n; i++) {
    var k = kl[i];
    var up = k.c >= k.o;
    var color = up ? '#ef5350' : '#66bb6a';
    var x = pad + i * candleW + candleW / 2;
    var yHigh = pad + (1 - (k.h - mn) / range) * ch;
    var yLow = pad + (1 - (k.l - mn) / range) * ch;
    var yO = pad + (1 - (k.o - mn) / range) * ch;
    var yC = pad + (1 - (k.c - mn) / range) * ch;
  ...
  return '<svg width="' + w + '" height="' + h + '" viewBox="0 0 ' + w + ' ' + h + '" style="display:block;cursor:pointer;" onclick="showMarketKline(' + onClickIdx + ')">' + parts.join('') + '</svg>';
}
function _renderMarketKlines() {
  fetch(API + '/api/market-kline?_t=' + Date.now()).then(function(r){ return r.json(); }).then(function(d){
    if (!d.ok || !d.klines) return;
    _marketKlineData = d.klines;
    var containers = document.querySelectorAll('.marketKline');
    for (var i = 0; i < d.klines.length; i++) {
      var kl = d.klines[i].klines || [];
      var svg = _buildKlineSvg(kl, 160, 36, i);
      if (containers[i]) containers[i].innerHTML = svg;
    }
  }).catch(function(){});
}
/** 大盘30日K线弹窗（K线 + 成交量 + 十字光标） */
function showMarketKline(idx) {
  var t = _marketKlineData[idx];
  if (!t || !t.klines || t.klines.length < 2) return;
  var kl = t.klines;
  var name = t.name;
  var mn = Infinity, mx = -Infinity;
  for (var i = 0; i < kl.length; i++) {
  ...
  var range = mx - mn || 1;
  var vMax = 0;
  for (var i = 0; i < kl.length; i++) if (kl[i].v > vMax) vMax = kl[i].v;
  vMax = vMax || 1;
  var w = 520, h = 380, pad = 40, cw = w - pad * 2;
  var ch = 210, vh = 70, vGap = 22;
  var n = kl.length;
  var candleW = cw / n;
  var bodyW = Math.max(candleW * 0.62, 1.5);
  var parts = [];
  // 水平参考线（价格区）
  var gridRows = 5;
  for (var g = 0; g <= gridRows; g++) {
    var gy = pad + g / gridRows * ch;
    var gv = mx - range * g / gridRows;
    parts.push('<line x1="' + pad + '" y1="' + gy.toFixed(1) + '" x2="' + (pad + cw) + '" y2="' + gy.toFixed(1) + '" stroke="rgba(255,255,255,0.06)" stroke-width="1"/>');
    parts.push('<text x="' + (pad - 4) + '" y="' + (gy + 3).toFixed(1) + '" font-size="10" fill="#777" text-anchor="end">' + gv.toFixed(0) + '</text>');
  }
  // 蜡烛 + 成交量柱
  var pts = [];
  for (var i = 0; i < n; i++) {
    ...
    parts.push('<rect x="' + (x - bodyW / 2).toFixed(1) + '" y="' + bodyTop.toFixed(1) + '" width="' + bodyW.toFixed(1) + '" height="' + bodyH.toFixed(1) + '" fill="' + color + '"/>');
    // 成交量柱
    var vTop = pad + ch + vGap;
    var vH = Math.max(k.v / vMax * vh, 0.5);
    parts.push('<rect x="' + (x - bodyW / 2).toFixed(1) + '" y="' + (vTop + vh - vH).toFixed(1) + '" width="' + bodyW.toFixed(1) + '" height="' + vH.toFixed(1) + '" fill="' + color + '" fill-opacity="0.45"/>');
    pts.push({x: x, idx: i, k: k});
  }
  // 分隔线
  parts.push('<line x1="' + pad + '" y1="' + (pad + ch + vGap - 6).toFixed(1) + '" x2="' + (pad + cw) + '" y2="' + (pad + ch + vGap - 6).toFixed(1) + '" stroke="rgba(255,255,255,0.1)" stroke-width="1"/>');
  parts.push('<text x="' + (pad + 4) + '" y="' + (pad + ch + vGap - 10).toFixed(1) + '" font-size="10" fill="#777">成交量</text>');
  // 十字光标
  parts.push('<line id="mktKCrossX" x1="0" y1="0" x2="0" y2="0" stroke="#888" stroke-width="1" stroke-dasharray="4,3" opacity="0"/>');
  parts.push('<line id="mktKCrossY" x1="0" y1="0" x2="0" y2="0" stroke="#888" stroke-width="1" stroke-dasharray="4,3" opacity="0"/>');
  var svgHtml = '<svg id="mktKSvg" width="' + w + '" height="' + h + '" viewBox="0 0 ' + w + ' ' + h + '" style="display:block;margin:0 auto;background:transparent;cursor:crosshair;">' + parts.join('') + '</svg>';
  ...
  backdrop.innerHTML = '<div style="background:#1a1a1a;border:1px solid #333;border-radius:8px;padding:16px;max-width:580px;width:92%;" onclick="event.stopPropagation();">'
    + '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">'
    + '<span style="color:#e0e0e0;font-size:15px;font-weight:600;">' + name + ' - 30日K线</span>'
    ...
    + svgHtml + '</div>';
  var svg = document.getElementById('mktKSvg');
  var crossX = document.getElementById('mktKCrossX');
  var crossY = document.getElementById('mktKCrossY');
  ...
  svg.onmousemove = function(e) {
    ...
    var kk = nearest.k;
    var prevC = kl[nearest.idx - 1] ? kl[nearest.idx - 1].c : kk.o;
    var chg = kk.c - prevC;
    var pct = prevC ? (chg / prevC * 100) : 0;
    var sign = chg >= 0 ? '+' : '';
    var color = chg >= 0 ? '#ef5350' : '#66bb6a';
    tooltip.innerHTML = '<span style="color:#aaa;">' + kk.d + '</span> '
      + '<span style="color:#aaa;">开</span><span style="font-family:Consolas;color:#e0e0e0;">' + kk.o.toFixed(2) + '</span> '
      + '<span style="color:#aaa;">高</span><span style="font-family:Consolas;color:#e0e0e0;">' + kk.h.toFixed(2) + '</span> '
      + '<span style="color:#aaa;">低</span><span style="font-family:Consolas;color:#e0e0e0;">' + kk.l.toFixed(2) + '</span> '
      + '<span style="color:#aaa;">收</span><span style="font-family:Consolas;color:#e0e0e0;">' + kk.c.toFixed(2) + '</span> '
      ...
      + '<span style="color:#aaa;">量</span><span style="font-family:Consolas;color:#e0e0e0;">' + (kk.v / 100000000).toFixed(1) + '亿</span>';
    crossX.setAttribute('x1', nearest.x); crossX.setAttribute('x2', nearest.x);
    crossX.setAttribute('y1', pad); crossX.setAttribute('y2', pad + ch + vGap + vh);
    crossX.setAttribute('opacity', '0.5');
    crossY.setAttribute('y1', mouseY.toFixed(1));
    crossY.setAttribute('y2', mouseY.toFixed(1));
    crossY.setAttribute('x1', pad); crossY.setAttribute('x2', pad + cw);
    crossY.setAttribute('opacity', '0.5');
  };
...
function showMarketTrend(idx) {
  ...
  if (t.current_price !== undefined) {
    ...
    var dotY = pad + ch - (t.current_price - mn) / range * ch;
    currentDot = '<circle cx="' + lastP.x.toFixed(1) + '" cy="' + dotY.toFixed(1) + '" r="4" fill="' + lineColor + '" stroke="#fff" stroke-width="1"/>'
      + '<line x1="' + pad + '" y1="' + dotY.toFixed(1) + '" x2="' + (pad + cw) + '" y2="' + dotY.toFixed(1) + '" stroke="' + lineColor + '" stroke-width="1" stroke-dasharray="4,3" opacity="0.4"/>';
  }
...
var DIM_INFO = {
  'y1': {name:'近1年收益',desc:'近1个自然年净值复利收益(约243个交易日)',cat:'perf'},
  'm3': {name:'近3月收益',desc:'近66个交易日净值复利收益',cat:'perf'},
  'm1': {name:'近1月收益',desc:'近22个交易日净值复利收益',cat:'perf'},
  'f5': {name:'近一周收益',desc:'最近5个交易日净值复利收益',cat:'perf'},
  'sy6': {name:'近6月收益',desc:'近6个自然月净值复利收益(约119个交易日)',cat:'perf'},
  'sy2': {name:'近2年收益',desc:'近2个自然年净值复利收益(约484个交易日)',cat:'perf'},
  'sy3': {name:'近3年收益',desc:'近3个自然年净值复利收益(约728个交易日)',cat:'perf'},
  'annual_return': {name:'年化收益率',desc:'基于全部净值(约3年)的年化复合收益',cat:'perf'},
  'max_dd': {name:'最大回撤',desc:'基于全部净值(约3年)的最大回撤',cat:'risk'},
  'volatility': {name:'波动率',desc:'基于全部净值(约3年)的年化波动率',cat:'risk'},
  'max_loss_days': {name:'最大连跌天数',desc:'全部净值(约3年)最长连续下跌天数',cat:'risk'},
  'sharpe': {name:'夏普比率',desc:'超额年化收益÷年化波动率',cat:'quality'},
  'sortino': {name:'索提诺比率',desc:'超额年化收益÷下行波动率',cat:'quality'},
  'profit_ratio': {name:'盈亏比',desc:'平均盈利÷平均亏损(全部净值)',cat:'quality'},
  'win_rate': {name:'上行胜率',desc:'赚钱交易日占比(全部净值)',cat:'quality'},
  'recovery': {name:'修复系数',desc:'区间总收益÷最大回撤',cat:'quality'},
  'calmar': {name:'卡玛比率',desc:'年化收益÷最大回撤',cat:'quality'},
  'rate': {name:'费率',desc:'申购费率(%)',cat:'other'},
  'scale': {name:'基金规模',desc:'基金规模(亿)',cat:'other'},
  'institutional': {name:'机构持有比例',desc:'机构持有比例(%)',cat:'other'},
  'td': {name:'当日涨跌',desc:'当日实时涨跌幅(盘中估算/收盘净值)',cat:'perf'},
};
// 支持按窗口（计算周期）的维度：全部/近1年/近2年/近3年
var _WINDOW_DIMS = ['max_dd','volatility','max_loss_days','sharpe','sortino','calmar','recovery','win_rate','profit_ratio','annual_return'];
var _WINDOW_OPTS = [['all','全部'],['1y','近1年'],['2y','近2年'],['3y','近3年']];
function _windowSelectHtml(key, cur) {
  if (_WINDOW_DIMS.indexOf(key) < 0) return '<span style="color:#444;">—</span>';
  cur = cur || 'all';
  var opts = _WINDOW_OPTS.map(function(o){ return '<option value="' + o[0] + '"' + (o[0]===cur?' selected':'') + '>' + o[1] + '</option>'; }).join('');
  return '<select onchange="markDimsDirty()" style="background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);border-radius:4px;color:#ccc;padding:2px 4px;font-size:11px;">' + opts + '</select>';
}
function _dimDesc(key, desc, lookback) {
  if (_WINDOW_DIMS.indexOf(key) < 0) return desc;
  var lb = lookback || 'all';
  var lbl = {'all':'全部(约3年)', '1y':'近1年', '2y':'近2年', '3y':'近3年'}[lb] || '全部(约3年)';
  var base = {
...
async function loadDims(customDims) {
  ...
  el.innerHTML = '<div style="overflow-x:auto;"><table style="width:100%;border-collapse:collapse;font-size:11px;" id="dimsTable"><thead><tr style="background:#2a2a2a;">'
    ...
    + '<th style="padding:4px 6px;text-align:center;color:#888;border-bottom:1px solid #333;">窗口</th>'
    ...
    + function() {
        ...
        _CAT_ORDER.forEach(function(cat) {
          ...
          + '<td style="padding:3px 6px;border-bottom:1px solid #333;color:#666;font-size:11px;">' + htmlEscape(_dimDesc(dim.key, dim.desc, dim.lookback)) + '</td>'
          + '<td style="padding:3px 6px;border-bottom:1px solid #333;text-align:center;">' + _windowSelectHtml(dim.key, dim.lookback) + '</td>'
...
function doAddDim() {
  ...
  tr.innerHTML = '<td style="padding:3px 6px;text-align:center;border-bottom:1px solid #333;"><input type="checkbox" checked onchange="markDimsDirty()"></td>'
    ...
    + '<td style="padding:3px 6px;border-bottom:1px solid #333;text-align:center;">' + _windowSelectHtml(key, 'all') + '</td>'
    + '<td style="padding:3px 6px;text-align:center;border-bottom:1px solid #333;"><button onclick="toggleCurveEditor(this)" style="background:none;border:none;color:#42a5f5;cursor:pointer;font-size:14px;" title="调整评分曲线">📐</button></td>'
    + '<td style="padding:3px 6px;text-align:center;border-bottom:1px solid #333;"><button onclick="removeDim(this)" style="background:none;border:none;color:#ef5350;cursor:pointer;font-size:14px;">✖</button></td>';
  ...
  curveSubRow.innerHTML = '<td colspan="7" style="padding:6px 12px;">'
    + '<div style="font-size:11px;color:#888;margin-bottom:4px;">评分曲线断点（输入值 → 得分）</div>'
    + '<div style="display:flex;gap:12px;align-items:flex-start;">'
    ...
    var poll = setInterval(function() {
      fetch(API + '/api/heartbeat?_t=' + Date.now()).then(function(r){ return r.json(); }).then(function(hb) {
        ...
        if (recHb) {
          ...
          if (progText) progText.textContent = '📋 刷新涨跌 ' + (recHb.detail || '') + ' | ' + p + '%';
        }
      }).catch(function(){});
    }, 800);
    // 并发启动自选表请求（不阻塞下方补算轮询），就绪后渲染
    var ftPromise = fetch(API + '/api/fund-table' + '?_t=' + Date.now()).then(function(r){ return r.ok ? r.text() : null; }).catch(function(){ return null; });
    // 等待后台补算窗口指标完成（改维度窗口时），再刷新推荐表
    var t0 = Date.now();
    while (Date.now() - t0 < 120000) {
      var bf = null;
      try {
        var hb2 = await fetch(API + '/api/heartbeat?_t=' + Date.now()).then(function(r){ return r.json(); });
        bf = hb2 && hb2.heartbeats && hb2.heartbeats['fund-backfill'];
      } catch(e) {}
      if (!bf) break; // 无补算任务或已完成
      var bp = bf.total > 0 ? Math.round(bf.progress / bf.total * 100) : 0;
      if (progFill) progFill.style.width = Math.min(bp, 99) + '%';
      if (progPct) progPct.textContent = bp + '%';
      if (progText) progText.textContent = '🔧 补算窗口指标 ' + (bf.detail || '') + ' | ' + bp + '%';
      await new Promise(function(r){ setTimeout(r, 1200); });
    }
    // 渲染自选表（已就绪）
    try { var ftH = await ftPromise; if (ftH) { if (ftH.indexOf('<tbody>') > 0) { var ftE = document.getElementById('fundFullTable'); if (ftE) { ftE.innerHTML = ftH; document.getElementById('fundFullTableCard').style.display = ''; setupDragSort(); } } else { var ftE2 = document.getElementById('fundFullTable'); if (ftE2) { ftE2.innerHTML = ftH; document.getElementById('fundFullTableCard').style.display = ''; } } } } catch(e) {}
    // 刷新推荐表（补算后按新窗口评分）
...
async function saveDims() {
  ...
  rows.forEach(function(row) {
    ...
    var lbTd = row.querySelectorAll('td')[4];
    var lbSel = lbTd ? lbTd.querySelector('select') : null;
    const lookback = lbSel ? lbSel.value : 'all';
    ...
    dims.push({name: name, key: key, weight: parseInt(num.value) / 100, enabled: cb.checked, desc: _dimDesc(key, desc, lookback), lookback: lookback, curve: curvePoints, category: cat});
  });
  try {
...
async function resetDims() {
  ...
  const defaults = [
    {name:'近3月收益',key:'m3',weight:0.12,enabled:true,desc:'近66个交易日净值复利收益，中期趋势',curve:_defCurve('m3'),category:'perf'},
    {name:'近1月收益',key:'m1',weight:0.15,enabled:true,desc:'近22个交易日净值复利收益，捕捉短期动量',curve:_defCurve('m1'),category:'perf'},
    {name:'近1年收益',key:'y1',weight:0.09,enabled:true,desc:'近1个自然年净值复利收益(约243个交易日)，反映一年表现',curve:_defCurve('y1'),category:'perf'},
    {name:'近一周收益',key:'f5',weight:0.03,enabled:true,desc:'最近5个交易日净值复利收益，捕捉短期动量',curve:_defCurve('f5'),category:'perf'},
    {name:'近6月收益',key:'sy6',weight:0.06,enabled:true,desc:'近6个自然月净值复利收益(约119个交易日)',curve:_defCurve('sy6'),category:'perf'},
    {name:'近2年收益',key:'sy2',weight:0.05,enabled:true,desc:'近2个自然年净值复利收益(约484个交易日)，填补中期维度',curve:_defCurve('sy2'),category:'perf'},
    {name:'近3年收益',key:'sy3',weight:0.07,enabled:true,desc:'近3个自然年净值复利收益(约728个交易日)，看穿越牛熊能力',curve:_defCurve('sy3'),category:'perf'},
    {name:'年化收益率',key:'annual_return',weight:0.04,enabled:true,desc:'基于全部净值(约3年)的年化复合收益',curve:_defCurve('annual_return'),category:'perf'},
    {name:'最大回撤',key:'max_dd',weight:0.10,enabled:true,desc:'基于全部净值(约3年)的最大回撤',curve:_defCurve('max_dd'),category:'risk'},
    {name:'波动率',key:'volatility',weight:0.03,enabled:true,desc:'基于全部净值(约3年)的年化波动率，衡量震荡幅度',curve:_defCurve('volatility'),category:'risk'},
    {name:'最大连跌天数',key:'max_loss_days',weight:0.02,enabled:true,desc:'全部净值(约3年)最长连续下跌天数',curve:_defCurve('max_loss_days'),category:'risk'},
    {name:'夏普比率',key:'sharpe',weight:0.06,enabled:true,desc:'超额年化收益÷年化波动率',curve:_defCurve('sharpe'),category:'quality'},
    {name:'索提诺比率',key:'sortino',weight:0.06,enabled:true,desc:'超额年化收益÷下行波动率，更贴近风险感受',curve:_defCurve('sortino'),category:'quality'},
    {name:'盈亏比',key:'profit_ratio',weight:0.07,enabled:true,desc:'平均盈利÷平均亏损，>1说明赚比亏多',curve:_defCurve('profit_ratio'),category:'quality'},
    {name:'上行胜率',key:'win_rate',weight:0.07,enabled:true,desc:'赚钱交易日占总交易日比例',curve:_defCurve('win_rate'),category:'quality'},
    {name:'修复系数',key:'recovery',weight:0.04,enabled:true,desc:'区间总收益÷最大回撤，衡量跌后恢复能力',curve:_defCurve('recovery'),category:'quality'},
    {name:'卡玛比率',key:'calmar',weight:0.03,enabled:true,desc:'年化收益÷最大回撤，衡量收益/风险比',curve:_defCurve('calmar'),category:'quality'},
    {name:'费率',key:'rate',weight:0.03,enabled:true,desc:'申购费率(%)，越低越好',curve:_defCurve('rate'),category:'other'},
    {name:'基金规模',key:'scale',weight:0.02,enabled:true,desc:'基金规模(亿)，1~50亿最理想',curve:_defCurve('scale'),category:'other'},
    {name:'机构持有比例',key:'institutional',weight:0.02,enabled:true,desc:'机构持有比例(%)，专业机构认可度',curve:_defCurve('institutional'),category:'other'},
    {name:'当日涨跌',key:'td',weight:0.03,enabled:true,desc:'当日实时涨跌幅(盘中估算/收盘净值)，捕捉盘中动量',curve:_defCurve('td'),category:'perf'},
  ];
  const r = await fetch(API + '/api/dims', {
    method: 'POST',
    headers: {'Content-Type': 'application/json'},
...
function addFilterCondition(field, op, value) {
  var condDiv = document.getElementById('filterConditions');
  if (!condDiv) return;
  var fieldOpts = {y1:'近1年收益',sy6:'近6月收益',m3:'近3月收益',m1:'近1月收益',sy2:'近2年收益',sy3:'近3年收益',f5:'近一周收益',sc:'规模(亿)'};
  ...
  var unit = (field === 'sc') ? '亿' : '%';
  html += '<span style="color:#888;font-size:12px;">' + unit + '</span>';
...
async function runRecommendFromFilter() {
  ...
  try {
    ...
    // 显示运行模式与预计时长（产品友好提示）
    if (d.mode_text) {
      if (statusEl) statusEl.textContent = '🚀 ' + d.mode_text + (d.est_text ? '，' + d.est_text : '');
    }
    ...
    var poll = setInterval(async function() {
      ...
      try {
        ...
        if (recAlive && recData && !recDone) {
          ...
          var icon = phaseIcon[recData.phase] || '⏳';
          // 友好阶段文案
          var friendlyPhase = {'获取排行':'获取市场数据','初筛':'初步筛选','限购':'检查限购','评分':'分析基金',
                               '保存':'保存结果','刷新涨跌':'更新今日行情','更新涨跌':'更新今日行情',
                               '涨跌':'更新行情','重新评分':'重新评分','检查自选基金':'补充自选基金',
                               '重新筛选':'重新筛选','预取持仓':'准备数据','预取行情':'拉取行情'};
          var fPhase = friendlyPhase[recData.phase] || recData.phase;
          // 剩余时间估算（按进度线性外推）
          var remainText = '';
          if (recData.overall_pct != null && recData.overall_pct > 1 && recData.overall_pct < 100 && recData.elapsed != null) {
            var remainSec = recData.elapsed / recData.overall_pct * (100 - recData.overall_pct);
          ...
          if (statusEl) statusEl.textContent = '⏳ 渲染表格...';
          if (btn) btn.textContent = '⏳ 渲染表格';
          // 结果文件已保存完毕（_supplement_self_selected 在保存前已执行），仅稍等磁盘落盘后并行拉取两张表
...
function showEstError(code) {
  var el = document.getElementById("estErrorModal");
  if (!el) {
    el = document.createElement("div");
    el.id = "estErrorModal";
    document.body.appendChild(el);
  }
  el.innerHTML = '<div style="background:#1a1a1a;border:1px solid #333;border-radius:8px;padding:14px 16px;font-size:12px;color:#ccc;max-width:560px;width:92%;">'
    + '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;white-space:nowrap;">'
    + '<div style="display:flex;align-items:baseline;gap:8px;min-width:0;"><span style="font-size:13px;font-weight:600;color:#e0e0e0;white-space:nowrap;">📈 持仓估算 vs 实际净值差异</span> <span id="estErrorTitle" style="color:#666;font-size:11px;font-family:Consolas;white-space:nowrap;">' + code + '</span></div>'
    + '<span onclick="document.getElementById(\'estErrorModal\').style.display=\'none\'" style="cursor:pointer;color:#555;font-size:16px;margin-left:8px;flex-shrink:0;white-space:nowrap;">&times;</span></div>'
    + '<div id="estErrorContent" style="padding:4px 0;color:#888;">⏳ 加载中...</div>'
    + '</div>';
  el.style.cssText = "position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);z-index:9999;max-height:90vh;overflow-y:auto;width:min(92vw,560px);";
  fetch(API + '/api/est-error?code=' + encodeURIComponent(code))
    .then(function(r){ return r.json(); })
    .then(function(d){
      var content = document.getElementById('estErrorContent');
      if (!content) return;
      // 标题显示基金名称（代码）
      var titleEl = document.getElementById('estErrorTitle');
      if (titleEl && d.name) titleEl.textContent = d.name + '（' + code + '）';
      if (!d.ok) { content.innerHTML = '<span style="color:#ef5350;">' + (d.error || '获取失败') + '</span>'; return; }
      if (!d.detail || d.detail.length === 0) {
        content.innerHTML = '<span style="color:#666;">暂无数据（盘中显示估算，收盘后结算即可查看差异）</span>';
        return;
      }
      var maeColor = d.mae < 1.0 ? '#66bb6a' : (d.mae <= 2.0 ? '#ffa726' : '#ef5350');
      var maeLabel = d.mae < 1.0 ? '准' : (d.mae <= 2.0 ? '中' : '偏差大');
      // 总计方向：净累计 = Σ(实际-估算)，正=总共好于估算，负=总共差于估算
      var sumErr = 0;
      for (var _i = 0; _i < d.detail.length; _i++) { sumErr += d.detail[_i].err; }
      var meanErr = sumErr / d.detail.length;
      var totalColor = sumErr >= 0 ? '#66bb6a' : '#ef5350';
      var totalText = sumErr >= 0 ? '好' : '差';
      var html = '<div style="margin-bottom:10px;padding:8px;background:rgba(0,0,0,0.3);border-radius:6px;text-align:center;">'
        + '<div style="white-space:nowrap;">近' + d.count + '天实际总共比估算<span style="color:' + totalColor + ';font-weight:600;">' + totalText + '</span> <span style="font-family:Consolas;font-weight:600;color:' + totalColor + ';">' + (sumErr >= 0 ? '+' : '') + sumErr.toFixed(2) + '%</span></div>'
        + '<div style="margin-top:4px;font-size:11px;color:#888;white-space:nowrap;">平均每天 ' + (meanErr >= 0 ? '+' : '') + meanErr.toFixed(2) + '% · 绝对误差 ±' + d.mae + '% ' + maeLabel + '</div>'
        + '</div>'
        + '<table style="width:100%;border-collapse:collapse;font-size:11px;white-space:nowrap;">'
        + '<thead><tr style="background:#2a2a2a;"><th style="padding:4px 8px;text-align:left;color:#888;border-bottom:1px solid #444;white-space:nowrap;">日期</th>'
        + '<th style="padding:4px 8px;text-align:right;color:#888;border-bottom:1px solid #444;white-space:nowrap;" title="盘中持仓估算涨跌">估算</th>'
        + '<th style="padding:4px 8px;text-align:right;color:#888;border-bottom:1px solid #444;white-space:nowrap;" title="收盘后实际净值涨跌">实际</th>'
        + '<th style="padding:4px 8px;text-align:right;color:#888;border-bottom:1px solid #444;white-space:nowrap;">误差</th></tr></thead><tbody>';
      d.detail.forEach(function(it){
        var est = it.est, actual = it.actual, err = it.err;
        // 方向+大小结合：好(正误差,实际好于估算)绿色系，差(负误差)红色系；|误差|越大颜色越深
        var errAbs = Math.abs(err);
        var errColor;
        if (err >= 0) {
          errColor = errAbs < 0.5 ? '#81c784' : (errAbs <= 1.0 ? '#66bb6a' : '#1e7d34');
        } else {
          errColor = errAbs < 0.5 ? '#e57373' : (errAbs <= 1.0 ? '#ef5350' : '#c62828');
        }
        html += '<tr><td style="padding:3px 8px;border-bottom:1px solid #333;color:#888;font-family:Consolas;white-space:nowrap;">' + it.date + '</td>'
          + '<td style="padding:3px 8px;text-align:right;border-bottom:1px solid #333;font-family:Consolas;color:#ccc;white-space:nowrap;">' + (est != null ? est.toFixed(2) + '%' : '-') + '</td>'
          + '<td style="padding:3px 8px;text-align:right;border-bottom:1px solid #333;font-family:Consolas;color:#ccc;white-space:nowrap;">' + (actual != null ? actual.toFixed(2) + '%' : '-') + '</td>'
          + '<td style="padding:3px 8px;text-align:right;border-bottom:1px solid #333;font-family:Consolas;font-weight:600;color:' + errColor + ';white-space:nowrap;" title="绿=实际好于估算,红=实际差于估算;颜色越深|误差|越大">' + (err >= 0 ? '+' : '') + err.toFixed(2) + '%</td></tr>';
      });
      html += '</tbody></table>';
      content.innerHTML = html;
    })
    .catch(function(){ var c = document.getElementById('estErrorContent'); if (c) c.innerHTML = '<span style="color:#ef5350;">请求失败</span>'; });
}
...
var _HLD_DIM_DESC = {
  ...
  net_profit_margin:'【绝对标准】销售净利率。净利润÷营收，每元营收能赚多少利润。>15%优秀。越高越好。自动校准不改变此曲线。',
...
function showHldScoreDetail(stockName, stockCode, tdEl) {
  ...
  dims.forEach(function(d){
    ...
    html += '<tr><td style="padding:3px 6px;border-bottom:1px solid #333;color:#e0e0e0;cursor:help;" title="' + htmlEscape(_HLD_DIM_DESC[d.k] || '') + '">' + htmlEscape(d.n) + (_HLD_LOCKED_KEYS[d.k] ? ' <span style="color:#888;font-size:9px;">🔒</span>' : '') + (isNull ? ' <span style="color:#555;">(中性)</span>' : '') + (src ? ' <span style="color:' + srcColor + ';font-size:8px;border:1px solid ' + srcColor + ';border-radius:2px;padding:0 3px;cursor:help;" title="' + htmlEscape(srcTip) + '">' + srcLabel + '</span>' : '') + '</td>'
...
// 走势图放大弹窗（支持近1月/近3月/近6月/近1年周期切换）
var _trendModal = { svg:null, crosshair:null, tooltip:null, pts:[], w:400, h:200, pad:30, code:'', name:'' };
var _TREND_RANGES = [
  {label:'近1月', days:22},
  {label:'近3月', days:66},
  {label:'近6月', days:125},
...
function showTrendChart(el) {
  ...
  _trendModal.name = el.getAttribute('data-name') || '走势图';
  _trendModal.code = el.getAttribute('data-code') || '';
  _openTrendModal();
  _renderTrend(dates, dayVals);
}
function _openTrendModal() {
  var backdrop = document.getElementById('trendBackdrop');
  if (!backdrop) {
    backdrop = document.createElement('div');
    backdrop.id = 'trendBackdrop';
  ...
  backdrop.style.display = 'flex';
  var btnHtml = '';
  for (var i = 0; i < _TREND_RANGES.length; i++) {
    var r = _TREND_RANGES[i];
    btnHtml += '<button data-days="' + r.days + '" onclick="switchTrendRange(this,' + r.days + ')" style="margin:0 3px;padding:3px 12px;border-radius:4px;border:1px solid #444;background:rgba(255,255,255,0.04);color:#ccc;font-size:12px;cursor:pointer;">' + r.label + '</button>';
  }
  backdrop.innerHTML = '<div style="background:#1a1a1a;border:1px solid #333;border-radius:8px;padding:16px;max-width:520px;width:92%;" onclick="event.stopPropagation();">'
    + '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">'
    + '<span style="color:#e0e0e0;font-size:14px;font-weight:600;">' + _trendModal.name + '</span>'
    + '<span onclick="document.getElementById(\'trendBackdrop\').style.display=\'none\'" style="cursor:pointer;color:#555;font-size:20px;">&times;</span></div>'
    + '<div style="text-align:center;margin-bottom:6px;" id="trendRangeBtns">' + btnHtml + '</div>'
    + '<div id="trendTooltip" style="text-align:center;font-size:13px;color:#e0e0e0;min-height:22px;margin-bottom:6px;">\u79fb\u52a8\u9f20\u6807\u67e5\u770b</div>'
    + '<svg id="trendSvg" width="' + _trendModal.w + '" height="' + _trendModal.h + '" viewBox="0 0 ' + _trendModal.w + ' ' + _trendModal.h + '" style="display:block;margin:0 auto;background:transparent;cursor:crosshair;"></svg>'
    + '<div id="trendLoadTip" style="text-align:center;font-size:11px;color:#666;min-height:14px;margin-top:6px;"></div>'
    + '</div>';
  _trendModal.svg = document.getElementById('trendSvg');
  _trendModal.tooltip = document.getElementById('trendTooltip');
}
function _renderTrend(dates, dayVals) {
  if (!dates || !dayVals || dayVals.length < 2) return;
  // 累计用复利（连乘）计算，与数据表"近1年收益"等净值收益一致；简单相加会高估/低估波动大的基金
  var cumVals = [], cum = 1;
  for (var vi = 0; vi < dayVals.length; vi++) { cum *= (1 + dayVals[vi] / 100); cumVals.push((cum - 1) * 100); }
  var minV = Math.min.apply(null, cumVals), maxV = Math.max.apply(null, cumVals), range = maxV - minV || 1;
  var w = _trendModal.w, h = _trendModal.h, pad = _trendModal.pad, cw = w - pad * 2, ch = h - pad * 2;
  var pts = [];
  ...
  var polyPts = pts.map(function(p){ return p.x.toFixed(1) + ',' + p.y.toFixed(1); }).join(' ');
  var svg = _trendModal.svg;
  if (!svg) return;
  svg.innerHTML = '<polyline fill="none" stroke="' + lineColor + '" stroke-width="2" points="' + polyPts + '"/>'
    + '<polygon fill="' + lineColor + '" fill-opacity="0.08" points="' + polyPts + ' ' + (pad+cw) + ',' + (pad+ch) + ' ' + pad + ',' + (pad+ch) + '"/>'
    + '<line id="trendCrosshair" x1="0" y1="0" x2="0" y2="0" stroke="#888" stroke-width="1" stroke-dasharray="4,3" opacity="0"/>';
  _trendModal.crosshair = document.getElementById('trendCrosshair');
  var crosshair = _trendModal.crosshair;
  var tooltip = _trendModal.tooltip;
  svg.onmousemove = function(e) {
    var rect = svg.getBoundingClientRect();
    var scaleX = w / rect.width;
    var mouseX = (e.clientX - rect.left) * scaleX;
...
function switchTrendRange(btn, days) {
  var tip = document.getElementById('trendLoadTip');
  var btns = document.querySelectorAll('#trendRangeBtns button');
  for (var i = 0; i < btns.length; i++) {
    btns[i].style.borderColor = '#444';
    btns[i].style.color = '#ccc';
    btns[i].style.background = 'rgba(255,255,255,0.04)';
  }
  btn.style.borderColor = '#42a5f5';
  btn.style.color = '#fff';
  btn.style.background = 'rgba(66,165,245,0.15)';
  if (tip) tip.textContent = '\u52a0\u8f7d\u4e2d\u2026';
  if (!_trendModal.code) { if (tip) tip.textContent = '\u7f3a\u5c11\u57fa\u91d1\u4ee3\u7801\uff0c\u65e0\u6cd5\u5207\u6362\u5468\u671f'; return; }
  fetch(API + '/api/fund-trend?code=' + encodeURIComponent(_trendModal.code) + '&days=' + days + '&_t=' + Date.now())
    .then(function(r){ return r.json(); })
    .then(function(d){
      if (!d.ok) { if (tip) tip.textContent = '\u83b7\u53d6\u5931\u8d25\uff1a' + (d.error || ''); return; }
      if (!d.d || !d.v || d.v.length < 2) { if (tip) tip.textContent = '\u6570\u636e\u4e0d\u8db3'; return; }
      if (tip) tip.textContent = '\u8fd1 ' + d.period + ' \u4e2a\u4ea4\u6613\u65e5';
      _renderTrend(d.d, d.v);
    })
    .catch(function(){ if (tip) tip.textContent = '\u8bf7\u6c42\u5931\u8d25'; });
}
