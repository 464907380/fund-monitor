...
def _compare_with_recommendations(held_rows: list[dict]) -> list[str]:
    ...
    lines.append("")
    lines.append("  ── 排名依据：11 维评分体系 ──")
    lines.append("  1️⃣  年化收益率（权重 10%）— 基金成立以来的年化回报，衡量长期赚钱能力")
    lines.append("  2️⃣  夏普比率（权重 15%）— 每承受 1 份波动能换来多少超额收益，越高说明性价比越好")
    lines.append("  3️⃣  索提诺比率（权重 10%）— 跟夏普类似但只考虑下跌波动，更贴近真实风险感受")
    lines.append("  4️⃣  最大回撤（权重 10%）— 历史最大跌幅，控制风险的能力")
    lines.append("  5️⃣  上行胜率（权重 5%）— 赚钱天数占总交易天数的比例")
    lines.append("  6️⃣  盈亏比（权重 5%）— 平均盈利 ÷ 平均亏损，> 1 说明赚的时候比亏的时候多")
    lines.append("  7️⃣  修复系数（权重 10%）— 总收益 ÷ 最大回撤，跌下去能不能快速涨回来")
    lines.append("  8️⃣  近3年收益（权重 20% 最高）— 最近 3 年的表现，权重最大，看基金能否穿越牛熊")
    lines.append("  9️⃣  机构持有比例（权重 5%）— 专业机构认可度，机构持有多的通常更稳")
    lines.append("  🔟  基金规模（权重 5%）— 1~50 亿最理想，太小不灵活、太大难操作")
    lines.append("  1️⃣1️⃣ 费率（权重 10%）— 申购费越低越好，省到就是赚到")
