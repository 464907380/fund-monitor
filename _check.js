  ...
  fetch(API + '/api/holdings?code=' + code)
    ...
    .then(function(d){
      ...
      d.holdings.forEach(function(h){
        ...
        if (h.fgl_date) {
          var fglDate = new Date(h.fgl_date);
          var now = new Date();
          var monthsAgo = (now.getFullYear() - fglDate.getFullYear()) * 12 + (now.getMonth() - fglDate.getMonth());
          if (monthsAgo <= 4) { freshnessColor = '#66bb6a'; freshnessTitle = '财报' + h.fgl_date + ' (较新)'; }
          else if (monthsAgo <= 7) { freshnessColor = '#ffa726'; freshnessTitle = '财报' + h.fgl_date + ' (偏旧)'; }
          else { freshnessColor = '#ef5350'; freshnessTitle = '财报' + h.fgl_date + ' (陈旧)'; }
        }
...
function showHldScoreDetail(stockName, stockCode, tdEl) {
  ...
  dims.forEach(function(d){
    ...
    if (src === '财报' && h.fgl_date) {
      srcLabel = '财报' + h.fgl_date;
      srcTip = '基于' + h.fgl_date + '季度财报数据，滞后1-3个月';
    }
