    ...
    var poll = setInterval(async function() {
      ...
      try {
        ...
        if (recAlive && recData && !recDone) {
          ...
          clearInterval(poll);
          // 子进程结束，开始刷新实时涨跌（用 fund_recommend 心跳看进度）
          if (cancelBtn) cancelBtn.style.display = 'none';
          if (prog) prog.style.width = '100%';
          if (statusEl) statusEl.textContent = '⏳ 刷新实时涨跌...';
          if (btn) btn.textContent = '⏳ 刷新涨跌';
          ...
          poll = setInterval(async function() {
            ...
            try {
              ...
              if (tdAlive && rec2) {
                ...
                // 没有 fund_recommend 心跳，可能是 td 刷新已完成且心跳被清除
                clearInterval(poll);
                if (prog) prog.style.width = '100%';
                if (statusEl) statusEl.textContent = '⏳ 渲染表格...';
                if (btn) btn.textContent = '⏳ 渲染表格';
  ...
  fetch(API + '/api/holdings?code=' + code)
    ...
    .then(function(d){
      ...
      d.holdings.forEach(function(h){
        ...
        // 数据新鲜度标记（按季度）
        var freshnessColor = '#888';
        var freshnessTitle = '无财报数据';
        var fglQuarter = '';
        if (h.fgl_date) {
          // 从日期算季度
          var fglM = parseInt(h.fgl_date.substring(5, 7), 10);
          var fglY = parseInt(h.fgl_date.substring(0, 4), 10);
          if (fglM <= 3) { fglQuarter = fglY + 'Q1'; }
          else if (fglM <= 6) { fglQuarter = fglY + 'Q2'; }
          else if (fglM <= 9) { fglQuarter = fglY + 'Q3'; }
          else { fglQuarter = fglY + 'Q4'; }
          // 当前预期的最新完整季度（按财报披露节奏）
          var now = new Date();
          var curM = now.getMonth() + 1;
          var curY = now.getFullYear();
          var expY = curY, expQ = 0;
          if (curM <= 4) { expY = curY - 1; expQ = 4; }      // 1-4月：预期最新是上年年报(Q4)
          else if (curM <= 8) { expY = curY; expQ = 1; }      // 5-8月：预期最新是Q1（半年报8月底截止）
          else if (curM <= 10) { expY = curY; expQ = 2; }     // 9-10月：预期最新是Q2（三季报10月底截止）
          else { expY = curY; expQ = 3; }                      // 11-12月：预期最新是Q3
          // 实际数据的季度序号
          var fglQ = parseInt(fglQuarter.slice(-1), 10);
          var qDiff = (expY - fglY) * 4 + (expQ - fglQ);
          if (qDiff <= 0) { freshnessColor = '#66bb6a'; freshnessTitle = fglQuarter + ' ✓'; }
          else if (qDiff <= 1) { freshnessColor = '#ffa726'; freshnessTitle = fglQuarter + ' (滞后1季)'; }
          else { freshnessColor = '#ef5350'; freshnessTitle = fglQuarter + ' (滞后' + qDiff + '季)'; }
        }
        // 文本季度标签颜色：Q1青, Q2绿, Q3橙, Q4紫
        var qColors = {'Q1':'#4dd0e1','Q2':'#66bb6a','Q3':'#ffa726','Q4':'#ab47bc'};
        var qColor = qColors[fglQuarter ? fglQuarter.slice(-2) : ''] || '#888';
        ...
        + '<span style="cursor:default;color:' + freshnessColor + ';font-size:10px;" title="' + freshnessTitle + '">●</span>'
        + (fglQuarter ? ' <span style="font-size:9px;font-weight:600;color:' + qColor + ';cursor:help;" title="' + freshnessTitle + '">' + fglQuarter + '</span>' : '')
        + ' <span onclick="event.stopPropagation();showStockInfo(\'' + htmlEscape(h.n || '') + '\',\'' + stockFullCode + '\')" style="cursor:pointer;border-bottom:1px dashed rgba(255,255,255,0.25);" title="财报:' + (h.fgl_date || '无') + '\nK线:最新交易日\n行情:今日实时\n上市：' + (h.listing_date || '-') + '\n行业：' + (h.industry || '-') + '\n点击查看详情">' + htmlEscape(h.n || '') + '</span></td>'
      ...
      (function(){
        ...
        function _applySavedOrder(saved) {
          ...
          order.forEach(function(name, pos){
            ...
            for (var i = 0; i < allThs.length; i++) {
              if (allThs[i].textContent === name && i !== pos) {
                var ref = allThs[pos];
                if (pos === 0) {
                  headerRow.insertBefore(allThs[i], headerRow.firstChild);
                } else if (i > pos) {
                  headerRow.insertBefore(allThs[i], ref.nextSibling);
                } else {
                  headerRow.insertBefore(allThs[i], ref);
                }
                Array.from(tbody.querySelectorAll('tr')).forEach(function(tr){
                  var tds = tr.querySelectorAll('td');
                  if (tds.length <= Math.max(i, pos)) return;
                  var refTd = tds[pos];
                  if (pos === 0) {
                    tr.insertBefore(tds[i], tr.firstChild);
                  } else if (i > pos) {
                    tr.insertBefore(tds[i], refTd.nextSibling);
                  } else {
                    tr.insertBefore(tds[i], refTd);
                  }
                });
                break;
              }
        ...
        fetch(API + '/api/holdings-col-order').then(function(r){ return r.json(); }).then(function(d){
          if (d.ok && d.order) _applySavedOrder(d.order);
        }).catch(function(){});
...
function showHldScoreDetail(stockName, stockCode, tdEl) {
  ...
  var html = '<div style="background:#1a1a1a;border:1px solid #333;border-radius:8px;padding:16px;margin:8px 0;font-size:12px;color:#ccc;max-height:80vh;overflow-y:auto;">'
    ...
    + '📅 <span style="color:#42a5f5;">■</span>财报(季报) <span style="color:#ffa726;">■</span>K线(近交易日) <span style="color:#66bb6a;">■</span>行情(今日)'
    + ' — 数据不同期，仅供参考。'
    + '</div>'
    + '<table style="width:100%;border-collapse:collapse;font-size:11px;">'
    + '<thead><tr style="background:#2a2a2a;"><th style="padding:4px 6px;text-align:left;color:#888;border-bottom:1px solid #444;">维度</th>'
  ...
  dims.forEach(function(d){
    ...
    var srcColor = _HLD_SRC_COLORS[src] || '#888';
    var srcTip = _HLD_SRC_TIPS[src] || '';
    var srcLabel = src;
    // 财报维度追加具体季度
    if (src === '财报' && h.fgl_date) {
      ...
      var _q;
      if (_fglM <= 3) _q = 'Q1'; else if (_fglM <= 6) _q = 'Q2'; else if (_fglM <= 9) _q = 'Q3'; else _q = 'Q4';
      srcLabel = _fglY + '' + _q;
      srcTip = '基于' + _fglY + '年' + _q + '财报';
    }
