  ...
  fetch(API + '/api/holdings?code=' + code)
    ...
    .then(function(d){
      ...
      d.holdings.forEach(function(h){
        ...
        tbl += '<tr><td style="padding:3px 6px;border-bottom:1px solid #333;color:#e0e0e0;max-width:120px;overflow:hidden;text-overflow:ellipsis;position:sticky;left:0;z-index:1;background:#1a1a1a;"><span onclick="event.stopPropagation();showStockInfo(\'' + htmlEscape(h.n || '') + '\',\'' + stockFullCode + '\')" style="cursor:pointer;border-bottom:1px dashed rgba(255,255,255,0.25);" title="上市：' + (h.listing_date || '-') + '\n行业：' + (h.industry || '-') + '\n点击查看详情">' + htmlEscape(h.n || '') + '</span></td>'
          + '<td style="padding:3px 6px;text-align:center;border-bottom:1px solid #333;font-family:Consolas;color:#888;">' + htmlEscape(h.c || '') + '</td>'
          + '<td style="padding:3px 6px;text-align:center;border-bottom:1px solid #333;font-family:Consolas;font-weight:600;cursor:pointer;color:' + (h.hld_score != null ? (h.hld_score >= 80 ? '#66bb6a' : h.hld_score >= 60 ? '#42a5f5' : h.hld_score >= 40 ? '#ffa726' : '#ef5350') : '#888') + ';" onclick="event.stopPropagation();showHldScoreDetail(\'' + htmlEscape(h.n || '') + '\',\'' + htmlEscape(h.c || '') + '\',this)">' + (h.hld_score != null ? h.hld_score.toFixed(1) : '-') + '</td>'
...
function showHldScoreDetail(stockName, stockCode, tdEl) {
  ...
  var html = '<div style="background:#1a1a1a;border:1px solid #333;border-radius:8px;padding:16px;margin:8px 0;font-size:12px;color:#ccc;max-height:80vh;overflow-y:auto;">'
    ...
    + '<div style="font-size:10px;color:#555;margin-bottom:8px;border-bottom:1px solid #333;padding-bottom:6px;">'
    + '💡 财务数据来自最新财报，K线/行情为近期数据。各指标可能不同期，仅供参考。'
    + '</div>'
    + '<table style="width:100%;border-collapse:collapse;font-size:11px;">'
    + '<thead><tr style="background:#2a2a2a;"><th style="padding:4px 6px;text-align:left;color:#888;border-bottom:1px solid #444;">维度</th>'
  ...
  dims.forEach(function(d){
    ...
    var scoreColor = isNull ? '#ffa726' : (score >= 80 ? '#66bb6a' : score >= 40 ? '#ffa726' : '#ef5350');
    html += '<tr><td style="padding:3px 6px;border-bottom:1px solid #333;color:#e0e0e0;cursor:help;" title="' + htmlEscape(_HLD_DIM_DESC[d.k] || '') + '">' + htmlEscape(d.n) + (isNull ? ' <span style="color:#555;">(中性)</span>' : '') + '</td>'
